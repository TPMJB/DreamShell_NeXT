    ...
    *,
    ...
    for game in games:
        roms = [
            ...
            if rom.size is not None and rom.crc32
        ]
        if not roms:
            continue
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Convert a Redump-compatible Dreamcast DAT into the compact CRC "
            "database used by GD Ripper on a Dreamcast."
        )
    )
    parser.add_argument("dat", help="Redump XML/ClrMamePro DAT, optionally .zip or .gz")
    parser.add_argument("-o", "--output", required=True, help="Output redump.db path")
    return parser
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        games, source, catalog_format = iter_dat_games(args.dat)
