...
class Preview:
    def __init__(self,app,values,focus='',hidden=(),active_tab=None):
        ...
        if active_tab is not None:
            for i in range(5):
                ...
                tab.find('label').set('color','#080F23' if i==active_tab else '#EFF5FC')
        self.canvas=Image.new('RGB',(640*S,480*S),'#080F23');self.draw=ImageDraw.Draw(self.canvas)
    ...
    def render(self,e,ox=0,oy=0):
        ...
        if e.tag=='input' and e.get('type')=='text':
            self.rect(x,y,w,h,'#EFF5FC')
            self.text(name,self.values.get(name,e.get('value','')),x+6,y,w-12,h,e.get('font','body'),'#14202B')
        if e.tag=='progressbar':
            self.surface(e.get('bimage'),x,y,w,h)
            self.surface(e.get('pimage'),x,y,round(w*float(e.get('pos','0'))),h)
        ...
        if e.tag=='filemanager':
            ...
            for i,text in enumerate(entries):
                self.rect(x,y+i*28,w-20,28,'#472958' if i==2 and name=='filemgr-top' else '#141D36')
...
def main(folder):
    ...
    gdplay=Preview('gdplay',{'title1-txt':'RESIDENT EVIL','title2-txt':'CODE: Veronica','disc-type':'GD-ROM','disc-state':'Ready to play',
        ...
        'status':'A plays the disc. B returns to the menu. K-UI closes when a game starts.'},focus='play-btn').save(folder/'gd-play.png')
    pages=[['Native output:  Auto (detect cable)','Screen shape:  4:3 (640 x 480)','Scaling filter:  Sharp (nearest)'],
        ...
        ['Connect at startup:  Ethernet','Sync clock at startup:  On','Network configuration:  Open Network app','Devices:  SD yes / IDE no / VMU yes','Running from:  /sd/DS','Restore defaults:  Review before saving','Restart K-UI']]
    ...
    icons=Image.new('RGB',(1280,960),'#080F23');d=ImageDraw.Draw(icons)
    ...
    d.text((48,38),'K-UI icon family',font=titlefont,fill='#EFF5FC')
    ...
    sheet=Image.new('RGB',(2560,1920),'#080F23')
