...
PolyForm permits the noncommercial purposes described in its text. Making the
source public does not waive its conditions. Recipients must receive the license
text or its URL and the applicable `Required Notice:` lines in [NOTICE](../NOTICE).
The release includes the license and notices in `DS/doc/`.
...
Files in `lib/`, `modules/`, `sdk/`, and other directories can have their own
licenses. Their original license files and source headers remain applicable;
the top-level license does not replace them. For example, FatFs has its own
notice, and the bundled Libretro-derived database has CC BY-SA 4.0 attribution.
See [the catalog provenance](../applications/gd_ripper/redump-db.README).
...
Downloads are free. Optional [Ko-fi support](https://ko-fi.com/tpmjb) does not
purchase a software license, exclusive features, or commercial-use permission.
