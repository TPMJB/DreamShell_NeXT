...
| Application bug batch | Host regressions cover coroutine lifetime, directory scan counts, mixed stick/D-pad input, confirmation selection, Classic exclusion and Speedtest mismatch reporting. | The ISO Loader/VMU exceptions, File Manager reboot, launcher XML reload failure and SD verification mismatch remain open for the combined hardware retest. See [1.0 test follow-ups](1.0-test-followups.md). No new console package has been built for this batch. |
