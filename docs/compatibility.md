...
| K-UI 3.3 disc startup | The maintainer reported a SEGA-screen freeze with `K-UI_bootloader_v3.3.cdi`, later also reporting an old-disc failure and a no-SD freeze. Commit `87c47a5` passed the full SH-4 build and host CI; disassembly confirms its early-init fix is in the CDI. Old 3.2, original 3.3 and fixed 3.3 all pass sector EDC/ECC checks and reach KOS in emulation. | The console freeze remains unresolved. Emulation stops at an unsupported SCI receive operation, so a complete menu boot is not verified. See the [artifact checks and remaining physical comparisons](bootloader-startup-check.md). The sharper badge includes `github.com/TPMJB`. |
